...
function systemEndpoints(app) {
  ...
  app.post("/system/user", [validatedRequest], async (request, response) => {
    try {
      ...
      const { username, password, userLang, zipCode } = reqBody(request);
      ...
      if (userLang) {
        updates.userLang = User.validations.userLang(String(userLang));
      }
      if (zipCode) {
        updates.zipCode = User.validations.zipCode(String(zipCode));
      }
